...
function createCheckoutSales() {
  ...
  JSON_FILE="$DIR_FILES/$CNPJ_FILIAL/json/cadastro-pdv-$NUMERO_PDV.json"

	CHECKOUT=$(curl -s -X POST "$BASE_URL/checkouts/sales" \
...
function readTabpdv() {
  ...
  for COD_FILIAL in ${COD_FILIAIS[@]}; do
    ...
    for PDV in ${PDVS[@]}; do
      ...
      ARQ_CUPONS='2503 2603 2703'
      for ARQ_CUPON in ${ARQ_CUPONS[@]}; do
        ...
        echo "$DADOS_PDV"| while IFS='|' read -r NUMERO DATAINATIV; do
          ...
          if [[ ! -z $COMPANY_ID ]]; then
            ...
            number=$NUMERO \
            status="$STATUS" > $DIR_FILES/$CNPJ_FILIAL/json/cadastro-pdv-$NUMERO.json
          else
            echo -ne "O campo company_id nao pode ser nulo para PDV $NUMERO." >> $DIR_FILES/$CGCFIL99/error-`date +%Y-%m-%d`.log
          fi

        createCheckoutSales $CNPJ_FILIAL $NUMERO
      done
    done
  done
}
