...
function readFIPDV() {
  ...
  for COD_FILIAL in ${COD_FILIAIS[@]}; do
    ...
    for PDV in ${PDVS[@]}; do
      ...
      for DIA in {01..31}; do
        if [[ -e fi$DIA$MES$PDV.dbf ]]; then

          DADOS_CUPOM=$(cdbflites fi$DIA$MES$PDV.dbf /TRIM:all /DELETED- /FILTER:origem='VENDAS' /SELECT:OPERADOR','ORIGEM','CUPOM','DEBI_CRED','ORDEM','EMPCONV','NUMAUTORI','BINCARTAO','NSU','BANDEIRA','CNPJCRED','ESPECIE','CANCELADO','OBSERVACAO','DATA','HORARIO','CGCCPF','NOMECLI','VALOR','TIPODOC)
            echo "$DADOS_CUPOM"| \
            ...
            VALOR_TOTAL=$(cdbflites fi$DIA$MES$PDV.dbf /TRIM:all /DELETED- /FILTER:cupom=$CUPOM /FILTER:origem='VENDAS' /FILTER:debi_cred='C' /SUM:valor | tr -d ' ' | sed 's/ /|/g;s/.$//g')
            TROCO_TOTAL=$(cdbflites fi$DIA$MES$PDV.dbf /TRIM:all /DELETED- /FILTER:cupom=$CUPOM /FILTER:debi_cred='D' /SUM:valor | tr -d ' ' | sed 's/ /|/g;s/.$//g')
            DESCONTO=$(cdbflites fi$DIA$MES$PDV.dbf /TRIM:all /DELETED- /FILTER:cupom=$CUPOM FILTER:especie='DESCONTO' /FILTER:debi_cred='*' /SUM:valor | tr -d ' ' | sed 's/ /|/g;s/.$//g')
            ...
            if [[ ! -z $COMPANY_ID ]] && [[ ! -z $CUPOM ]] && [[ ! -z $NUM_PDV ]]; then
              ...
              function createCheckoutSalesTransactions() {
                ...
                TRANSACTIONS_CUPOM=$(cdbflites fi$DIA$MES$PDV.dbf /TRIM:all /DELETED- /FILTER:cupom=$CUPOM /SELECT:OPERADOR','ORIGEM','CUPOM','DEBI_CRED','ORDEM','EMPCONV','NUMAUTORI','BINCARTAO','NSU','BANDEIRA','CNPJCRED','ESPECIE','CANCELADO','OBSERVACAO','DATA','HORARIO','CGCCPF','NOMECLI','VALOR','TIPODOC)
